...
router.get('/:id', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (fileExt === '.txt' || fileExt === '.log') {
      ...
      // Warn if logcat sections had content but produced zero parsed entries
      if (allEntries.length === 0 && totalLines > 10) {
        sendSSE(res, {
          stage: 'error',
          progress: 0,
          message: `Logcat sections found (${totalLines} lines) but no entries could be parsed. The logcat data may be corrupted or in an unsupported format.`,
        });
        res.end();
        return;
      }
    ...
    // Detect corrupted ZIP errors from yauzl
...
function analyzeStandaloneFile(
  ...
  if (format === 'unknown') {
    sendSSE(res, {
      stage: 'error',
      progress: 0,
      message: 'Unable to detect file format. Expected logcat (e.g., "01-01 12:00:00.000  1000  1234  1234 I Tag : message") ' +
        'or dmesg (e.g., "<6>[    0.000000] Booting Linux") output. ' +
        'For full analysis, upload a .zip bugreport generated by `adb bugreport`.',
    });
    res.end();
    return null;
  }
  ...
  if (format === 'logcat') {
    ...
    if (logcatResult.entries.length === 0) {
      sendSSE(res, {
        stage: 'error',
        progress: 0,
        message: 'File was detected as logcat format but no entries could be parsed. ' +
          'Expected format: "01-01 12:00:00.000  1000  1234  1234 I Tag : message". ' +
          'For full analysis, upload a .zip bugreport generated by `adb bugreport`.',
      });
      res.end();
      return null;
