    ...
    <div className="card space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-surface rounded-lg p-3 space-y-3">
          ...
          {/* Doze settings vs AOSP defaults */}
          {dozeSettings && (dozeSettings.inactiveTo > 0 || dozeSettings.idleTo > 0) && (
            <div className="space-y-1 pt-1">
              <div className="text-xs text-gray-500">Doze Settings vs AOSP</div>
              <div className="space-y-0.5">
                ...
                ] as [string, number, number][]).map(([name, val, def]) => {
        ...
        <div className="bg-surface rounded-lg p-3 space-y-3">
          ...
          {bs && (
            <>
              ...
              {/* Deep Doze discharge rate - highlighted */}
              {bs.deepDozeTimeMs > 0 && (
                <div className={`rounded-lg p-2 ${rateBg(bs.deepDozeDischargeRateMahPerHr)}`}>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-400">Deep Doze Discharge Rate</span>
                    <span className={`text-sm font-bold ${rateColor(bs.deepDozeDischargeRateMahPerHr)}`}>
                      {bs.deepDozeDischargeRateMahPerHr.toFixed(1)} mAh/h
                    </span>
                  </div>
                  <div className="text-xs text-gray-500 mt-1">
                    {bs.deepDozeTime} | {bs.deepDozeDischargeMah.toFixed(1)} mAh
      ...
      {/* Top Kernel Wakelocks — full width below the 3-column grid */}
