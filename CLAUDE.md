...
```bash
npm run e2e -w packages/frontend          # Run all 38 e2e tests (Playwright Test, ~4min)
...
38 Playwright Test specs across 12 files covering: upload/analysis flow, system overview, insights filters, timeline+search integration, search modal (Log Viewer with virtual scroll, live filtering, Find navigation), search full-load (warm cache + cold cache/history path with FTS5 rebuild), history panel, export menu, power overview, section nav, LLM settings, landing page, and FTS5 SQL fallback verification. Uses `global-setup.ts` to upload a sample bugreport once and share the analysis across all tests. Config: chromium only, 1440×900 viewport, `workers: 1`, auto-starts backend+frontend via `webServer`. Backend dev-only `POST /api/_test/clear-raw-store/:id` endpoint enables FTS5 fallback path testing.
...
- `unpacker.ts` — ZIP extraction, section splitting (logcat with buffer info, ANR traces, kernel, dumpsys), HW/SW metadata extraction from SYSTEM PROPERTIES (platform, hardware, cpuAbi, serialNumber, basebandVersion, bootloaderVersion, securityPatchLevel, buildType, isDebuggable, isAdbSecure)
- `logcat-parser.ts` — 11 anomaly types (crash, ANR, watchdog, OOM via `Kill ` prefix, etc.); `parseLogcat(content, buffer?)` accepts optional buffer param; `detectAnomalies()` and `computeTagStats()` are exported for per-section parsing
- `anr-parser.ts` — 18 ANR case types, lock graph construction, deadlock detection
- `kernel-parser.ts` — 12 kernel event types; auto-detects dmesg vs `logcat -b kernel -v threadtime` format; returns `parseErrors` count for non-matching lines. `KernelParseResult.format` field indicates detected format. `parseLogcatKernel()` re-bases timestamps to eliminate negative offsets from clock correction jumps
...
- `telephony-parser.ts` — Telephony analysis: ServiceState snapshot (voice/data reg state, operator with MCC/MNC fallback lookup, RAT, roaming), SignalStrength snapshot (LTE/NR/WCDMA/GSM/CDMA metrics with Integer.MAX_VALUE filtering), OOS event detection (start/end pairing with duration calculation), RIL error detection (7 types with `anyLevel` flag), call/SMS events, RAT change tracking, `parseDumpsysOosPeriods()` (cumulative OOS from dumpsys phone updateDataRoamingStatus, filters <30s boot init), `parseDataNetworkOosPeriods()` (from DataNetworkController Local logs — disconnect/reconnect pairs with cause and duration), `parseModemRestartCount()` (from dumpsys isub SIM state UNKNOWN cycles), radio log buffer time range. **OOS data priority**: (1) RAT distribution `oos` entry from CONNECTIVITY POWER SUMMARY (time + %), (2) `dataNetworkOosPeriods` from dumpsys (event count + duration), (3) `dumpsysOosPeriods` legacy fallback. If neither (1) nor (2) exists, OOS card hidden. **RIL error level filter**: `RilErrorRule.anyLevel` bypasses W/E/F filter for critical events (modem_restart, radio_crash, modem_err) that Android logs at D/I level; noise-prone types (request_not_supported, radio_not_available) stay W/E/F only
- `oom-analyzer.ts` — OOM cross-analysis: parses `Low on memory:` dump (top process PSS/memtrack), `oom_reaper` entries, `am_low_memory` events buffer trend, LMK kills from logcat+kernel; `analyzeOom()` accepts `userDescription` for target app extraction and timestamp matching; result stored in `AnalysisResult.oomResult`
- `clock-correction.ts` — Clock correction detection: `detectClockCorrections(entries)` performs O(n) single-pass detection on sorted logcat entries using charCode-based timestamp parsing (no regex). Detects NTP/NITZ clock jumps > 1 hour after Android boot (device starts with wrong time from build date or epoch). Uses confirmation window (80% of next 30 entries must stay in new range) to filter cross-buffer interleaving false positives. Handles month-boundary and year-boundary wraps. Returns `ClockCorrectionResult` with jump details and pre-correction entry count. Pre-correction entries have unreliable timestamps but are NOT modified — only boundary index stored. Integrated into `analyzeBasic()` to generate `warning` severity insight card and timeline events.
- `basic-analyzer.ts` — Rule-based analysis, health scoring (stability/memory/responsiveness/kernel), insight card generation, timeline↔insight linking. `BasicAnalyzerInput.userDescription` passes user's problem description + quick tags to `analyzeOom()` for target app matching. OOM insight severity dynamic: critical (adj<900 or ≥5 LMK or ≥50 reaps), warning (any LMK or ≥10 reaps), info otherwise. Kernel insight/timeline timestamps have ±30 day sanity check from bootEpochMs. Transport error severity: ENODEV/device_gone is critical, TransportErrorCallback-only is warning. Clock correction timeline events filtered by valid date range. `kernelUptimeSeconds` only used for dmesg format
...
- `types.ts` — All shared type definitions used across packages (includes `LogcatBuffer`, `LogcatSection`, `BufferTimeRange`, `LogEntry.buffer?`, `BugreportMetadata.buildType/.platform/.hardware/.cpuAbi/.serialNumber/.basebandVersion/.bootloaderVersion/.securityPatchLevel/.isDebuggable/.isAdbSecure`, `KernelParseResult.parseErrors/.format`, `PowerManagerState`, `DozeState`, `DozeSettings`, `BatteryStatsSummary`, `KernelWakeLockStat`, `AlarmWakeupStat`, `SuspendStats`, `PowerParseResult`, `EstimatedPowerUse`, `ConnectivityStats`, `PartialWakeLockStat`, `TelephonyParseResult`, `ServiceStateSnapshot`, `SignalStrengthSnapshot`, `OosEvent`, `RilError`, `CallEvent`, `SmsEvent`, `RatChangeEvent`, `DumpsysOosPeriod`, `OomAnalysisResult`, `OomSummary`, `OomProcessInfo`, `ReapedProcess`, `LowMemoryTrend`, `LmkKill`, `ClockJump`, `ClockCorrectionResult`). `AnalysisResult.clockCorrection?` stores clock correction data; `AnalysisResult.bufferTimeRanges?` stores per-buffer time ranges.
...
- **Prompt Templates** (`llm-gateway/prompt-templates/`): `analysis.ts` builds deep analysis prompt, `chat.ts` builds follow-up prompts, `context-builder.ts` composes analysis context (OOM insights route to `collectOomContext()` for structured memory pressure data)
- **Database** (`db.ts`): SQLite via better-sqlite3, WAL mode. `analyses` table (includes `build_type`, `is_debuggable`, `is_adb_secure` columns) + `logcat_fts` FTS5 virtual table (with `pid`, `tid`, `buffer` columns) + `kernel_fts` FTS5 virtual table. FTS5 tables use DROP+CREATE on migration (no ALTER TABLE support). Schema migrations use `ALTER TABLE ADD COLUMN` in try/catch loop for idempotent column additions — new columns must be added to both `CREATE TABLE` and migrations array. **FTS5 orphan cleanup**: FTS5 tables retain data from deleted/failed analyses; on VPS with limited RAM, orphaned rows cause query timeouts — periodically `DELETE FROM logcat_fts WHERE analysis_id NOT IN (SELECT id FROM analyses)` then `VACUUM`
...
- **Search ordering**: FTS5 path returns results by `ORDER BY rank` (BM25 relevance); in-memory path (when rawDataStore available) preserves chronological order. SearchModal full-load always uses in-memory path → chronological
...
- `components/BufferExtractor.tsx` — Client-side ZIP buffer extraction component for standalone logcat/dmesg file uploads (fallback when parser unpacking fails)
...
- `components/SystemOverview.tsx` — Hero device heading (T70 + manufacturer) with tag badges (Android ver, build type, boot status, uptime, adb root/debuggable). Buffer time ranges displayed. Large Overall score ring (120px) + 4 dimension progress bars. Memory/CPU/HAL resource cards with usage bars. Collapsible "Show details" for HW & SW info. InsightCard LogSnippet supports text UID format (root/lmkd/radio)
- `components/HistoryPanel.tsx` — Slide-out analysis history browser, shows build type and root badges
...
- `components/ComparisonPage.tsx` — Full-page comparison view (not modal). Uses `'comparison'` AppPhase. Compare success → `setPhase('comparison')`, Back → `setPhase('result')`. 7 sections: Hero (device metadata + score rings), Health bars, Key Changes, Power diff, Telephony diff, HAL changes, ANR changes. Backend `comparison.ts` includes MetadataDiff, PowerDiff, TelephonyDiff.
...
- `components/SearchModal.tsx` — Log Viewer modal with react-window v2 virtual scroll (`FixedSizeList`, ROW_HEIGHT=22). Logcat/Kernel tab switcher. **Full-load architecture**: loads ALL entries at once via `export=true&limit=1000000` API on open, then all filtering (level/pid/buffer/tag/startTime/endTime) is purely client-side via useMemo — zero server round-trips after initial load. Loading animation (spinner + "Loading all entries...") during initial fetch. Find keyword with `Enter`/`Shift+Enter` for Next/Prev match navigation + `▲▼` buttons + match counter (`3/47`). Keyboard `←`/`→` arrow keys for page-up/page-down scroll (when not in input). `initialFocusTime` support: `▶` marker on closest entry in filteredEntries (not allEntries), auto-scrollToRow. CSV/Text export from client memory (synchronous). Saved tag presets in localStorage
- `components/Timeline.tsx` — Timeline event list with severity/source filters (Logcat/Kernel/ANR/Clock); hover reveals search icon (magnifying glass) that opens SearchModal with ±5s time window around the event timestamp (via `onSearchTime` prop). Clock source shows purple filter button for clock correction jump events. Original insightId click-to-scroll behavior preserved with `e.stopPropagation()`
...
- **SSE through Cloudflare CDN**: Cloudflare buffers SSE responses. Frontend `api.ts` has a stall-detector polling fallback: if no SSE event arrives within 4s, polls `/api/analyze/:id/result` every 2s with simulated progress. SSE and polling race — whichever delivers first wins.
...
- **SearchModal detail panel**: MUST use pure DOM manipulation (`ref.current.style.display`, `textContent`) — any `useState` triggers SearchModal re-render which re-runs all useMemo hooks (200K+ filteredEntries + matchIndices scan) and hangs the UI. Never add frequently-changing state to rowProps dependencies.
- **SearchModal focusIndex**: Must search through `filteredRef.current` (not `allEntriesRef.current`) since the virtual list renders `filteredEntries`. When startTime/endTime filters are active, indices in allEntries don't match indices in filteredEntries.
- **E2e fixture selector**: `analysis.fixture.ts` waits for `#section-overview` (not `h2` with text). When UI headings change, e2e fixtures/tests using text-based selectors break silently — prefer `id`-based selectors for structural waits
...
- **Theme CSS variables**: All theme colors use RGB triplet format (e.g., `--color-border: 28 38 64`) in `index.css`, consumed as `rgb(var(--color-border) / <alpha-value>)` in `tailwind.config.js`. This enables Tailwind opacity modifiers (`border-border/50`). Hex format breaks opacity. Light/dark toggle via `[data-theme="light"]` on `<html>`, persisted in localStorage, anti-flash script in `index.html`
...
- **VPS deployment**: Docker Compose at `/opt/logcat-ai/docker-compose.yml` on VPS (198.13.48.150). Domain `logcat-ai.zmlab.io` goes through Cloudflare. Container needs `mem_limit: 4g, memswap_limit: 6g` and `--max-old-space-size=3072` for large bugreports (VPS has 1.9GB RAM, uses swap). CI/CD via `.github/workflows/deploy.yml` → SSH to VPS → `deploy.sh`
- **VPS nginx**: `/etc/nginx/sites-available/logcat-ai` reverse proxies HTTPS → `127.0.0.1:8000` with Let's Encrypt SSL. When changing Docker port mapping, must also update nginx `proxy_pass`. SSE support configured (`proxy_buffering off`)
- **Logcat entry ordering**: `analyze.ts` and `batch.ts` sort merged `allEntries` by timestamp after parsing all logcat sections, so entries from different buffers (main, system, radio) are interleaved chronologically. SearchModal loads all entries at once, so sorting ensures all buffers are interleaved correctly
...
`BugreportMetadata.buildType` 從 `ro.build.type` system property 提取，值為 `user`、`userdebug`、`eng` 或 `unknown`。`BugreportMetadata.isDebuggable`（`ro.debuggable`）和 `isAdbSecure`（`ro.adb.secure`）標示裝置的 debug/root 能力。前端 `SystemOverview` 對非 `user` build 顯示黃色警示標籤，`debuggable + !adbSecure` 顯示紅色 `adb root` 標籤。History panel 也顯示 build type 和 root badge。

### Kernel Log 可用性（`ro.logd.kernel`）

AOSP `logd` 的 klogd daemon 是否讀取 `/dev/kmsg` 由 `ro.logd.kernel` property 控制（[AOSP logd README.property](https://android.googlesource.com/platform/system/logging.git/+/refs/tags/android-14.0.0_r36/logd/README.property)）：
- **預設 true**：當 `ro.config.low_ram == false` 且 `ro.debuggable == true`
- **預設 false**：其他情況（標準 user build）

| Build | `ro.debuggable` | `logcat -b kernel` | KERNEL LOG section |
|-------|----------------|---------------------|-------------------|
| user（標準）| 0 | **空** — klogd 不啟動 | 空 |
| user + debuggable | 1 | 有資料 | logcat 格式（有時鐘校正問題）|
| userdebug | 1 | 有資料 | dmesg 或 logcat 格式 |

`adb root`/`adb unroot` **不影響** `logcat -b kernel` 內容 — 由 `logd` daemon 的 SELinux policy + `ro.logd.kernel` 決定。
