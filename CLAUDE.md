...
- `clock-correction.ts` — Clock correction detection: `detectClockCorrections(entries)` performs O(n) single-pass detection on sorted logcat entries using charCode-based timestamp parsing (no regex). Detects NTP/NITZ clock jumps > 1 hour after Android boot (device starts with wrong time from build date or epoch). Skips first 10 entries to avoid cross-buffer interleaving false positives. Handles month-boundary and year-boundary wraps. Returns `ClockCorrectionResult` with jump details and pre-correction entry count. Pre-correction entries have unreliable timestamps but are NOT modified — only boundary index stored. Integrated into `analyzeBasic()` to generate `warning` severity insight card and timeline events.
- `basic-analyzer.ts` — Rule-based analysis, health scoring (stability/memory/responsiveness/kernel), insight card generation, timeline↔insight linking. `BasicAnalyzerInput.userDescription` passes user's problem description + quick tags to `analyzeOom()` for target app matching
