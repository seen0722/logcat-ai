...
- **Search `fullTimeRange`**: In-memory logcat/kernel search responses include `fullTimeRange: { first, last, total }` — the unfiltered dataset's time boundaries and entry count. Used by SearchModal to show loaded window position and compute "Latest" button offset
...
- **SearchModal detail panel**: MUST use pure DOM manipulation (`ref.current.style.display`, `textContent`) — any `useState` triggers SearchModal re-render which re-runs all useMemo hooks (50K filteredEntries + matchIndices scan) and hangs the UI. Never add frequently-changing state to rowProps dependencies.
...
- **Logcat entry ordering**: `analyze.ts` and `batch.ts` sort merged `allEntries` by timestamp after parsing all logcat sections, so entries from different buffers (main, system, radio) are interleaved chronologically. Without sorting, the first 50K entries in SearchModal may contain only one buffer
