...
- **Full-text search** — FTS5 BM25-ranked search for both logcat and kernel logs with in-app Search Modal (Logcat/Kernel tab, adaptive filters, severity color-coding) and CSV/Text export
...
GET    /api/search/:id          Search logcat/kernel entries (FTS5/keyword, source=logcat|kernel)
